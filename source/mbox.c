 ...
 *
 * We must ensure that our our buffer is located at a 16 byte aligned address, * so only the high 28 bits contain the address
...
void mailbox_send(uint32_t msg, unsigned char channel) {
  ...
  do {
    asm volatile("nop");
  } while (*MBOX1_STATUS & MBOX_FULL);  // send the message
  *MBOX1_WRITE = msg;
}
